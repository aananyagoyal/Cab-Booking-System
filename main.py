from tkinter import *  # name of the Python library used for creating graphical user interfaces (GUIs).
from tkinter import (  #  module provides functions for displaying various message boxes in your Tkinter application
    messagebox, ttk)

#Global variables to store user login information
loggeduser=""
loggedname=""
# Function to navigate to cab booking page
def gotocpage():
    global root
    root.destroy()
    cabbook()
...
def validate():
    ...
    usr=userEntered.get()
    pwd=passEntered.get()
    f=open("users.txt",'r') #Open user file in read mode
    users=list(f) # Read lines into a list
    f.close()
    for i in users:
        uname,passw,name,email,secq,secan=i.split('|') ## Splitting user data by '|'
        if usr==uname and pwd==passw: # If username and password match
            root.destroy()
            loggeduser=usr  # Storing logged-in user's username
            loggedname=name
            cabbook() # Navigate to cab booking page
            return
